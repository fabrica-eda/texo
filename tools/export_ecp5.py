...
def subtract_range(total, prefix):
    """Remove an already-accounted-for serial delay from a timing range."""
    return {
        "min_ps": total["min_ps"] - prefix["min_ps"],
        "max_ps": total["max_ps"] - prefix["max_ps"],
    }
...
def export_cell_timings(cell_database):
    ...
    for entry in carry:
        ...
        source = entry["from_pin"]
        destination = entry["to_pin"]
        raw_carry_arcs[(source, destination)] = delay_range(entry)

    carry_arcs = [{}, {}]
    for slice_index in range(2):
        suffix = str(slice_index)
        for source in "ABCD":
            for destination in ("F", "FCO"):
                carry_arcs[slice_index][(source, destination)] = raw_carry_arcs[
                    (f"{source}{suffix}", f"F{suffix}" if destination == "F" else "FCO")
                ]

    # Project Trellis characterizes SCCU2C as one two-LUT primitive. In the
    # split-slice graph, the first half owns the primitive FCI->FCO delay. The
    # second half must therefore contain only the remainder of FCI->F1 and a
    # zero-delay FCI->FCO arc. This is the same decomposition used by nextpnr's
    # ECP5 chipdb generator.
    carry_through = raw_carry_arcs[("FCI", "FCO")]
    carry_arcs[0][("FCI", "F")] = raw_carry_arcs[("FCI", "F0")]
    carry_arcs[0][("FCI", "FCO")] = carry_through
    carry_arcs[1][("FCI", "F")] = subtract_range(
        raw_carry_arcs[("FCI", "F1")], carry_through
    )
    carry_arcs[1][("FCI", "FCO")] = {"min_ps": 0, "max_ps": 0}
