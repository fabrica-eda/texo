...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let input: serde_json::Value = serde_json::from_reader(BufReader::new(File::open(&args[2])?))?;
    let requests: Vec<(String, String)> = serde_json::from_value(input["pips"].clone())?;
    let wanted_bels: BTreeSet<String> = serde_json::from_value(input["bels"].clone())?;
    let mut bels = BTreeMap::new();
    for bel in device.bels() {
        if wanted_bels.contains(&bel.name) {
            let pins: BTreeMap<_, _> = bel
                .pins()
                .iter()
                .map(|id| {
                    let pin = &device.bel_pins()[id.0];
                    (pin.name.clone(), device.wires()[pin.wire.0].name.clone())
                })
                .collect();
            bels.insert(bel.name.clone(), pins);
        }
    }
    if bels.len() != wanted_bels.len() {
        return Err("missing requested physical BEL".into());
    }
